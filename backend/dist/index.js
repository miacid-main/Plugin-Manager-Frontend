...
import { authLoginSchema, pluginConsoleSchema, pluginEventSchema, pluginHeartbeatSchema, pluginPlayersSchema, pluginRegisterSchema, pluginStatusSchema, serverCommandSchema, } from './schemas.js';
const app = express();
...
function requireRegisteredPlugin(pluginName, res) {
...
app.post('/plugin/heartbeat', (req, res) => {
    ...
    if (!body)
        return;
    if (!requireRegisteredPlugin(body.pluginName, res))
        return;
    const plugin = state.upsertPlugin({
        name: body.pluginName,
        description: body.pluginDescription,
    ...
    if (typeof body.onlinePlayers === 'number')
...
app.get('/servers/:serverId/console', requireAuth, (req, res) => {
    ...
    const onEvent = (payload) => writeSse(res, 'line', payload);
...
app.get('/servers/:serverId/players', requireAuth, (req, res) => {
    ...
    const onEvent = (payload) => writeSse(res, 'players', payload);
    sub.emitter.on('event', onEvent);
    const keepAlive = setInterval(() => res.write(':keep-alive\n\n'), 15_000);
    req.on('close', () => {
        clearInterval(keepAlive);
